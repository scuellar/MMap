...
Module WFacts_fun (E:DecidableType)(Import M:WMapsOn E).
...
Proof.
intros; 
apply find_spec in H; apply find_spec in H0; 
rewrite H in H0; inversion H0; auto.
Qed.

(** ** Specifications written using equivalences *)
(* TODO: For compatibility reasons, it makes sense for these to stay. Still, they seem weird 
         given that our specs are actually of the <-> form. Suggestions? *)
Section IffSpec.
...
apply (MapsTo_spec H H0); auto.
apply MapsTo_spec with (k:=y);
[apply E.eq_equiv | ]; auto.
Qed.

Lemma MapsTo_iff : forall m x y e, E.eq x y -> (MapsTo x e m <-> MapsTo y e m).
Proof.
split; apply MapsTo_spec; auto;
apply E.eq_equiv; auto.
Qed.

Lemma mem_in_iff : forall m x, In x m <-> mem x m = true.
Proof.
symmetry; apply mem_spec.
Qed.
...
split; auto.
symmetry; apply (MapsTo_fun (e':=e) H);
apply add_spec1; apply E.eq_equiv; auto.
split; auto; apply add_spec2 with x e; auto.
intro; apply E.eq_equiv in H0; auto.
subst; apply add_spec1; apply E.eq_equiv; auto.
apply add_spec2; auto. intro; apply E.eq_equiv in H0; auto.
Qed.
...
right; exists e'; auto.
assert (~ E.eq y x) by (intro; apply E.eq_equiv in H0; auto). 
apply add_spec2 with (A:=elt) (m:=m) (v:=e') (v':=e) in H0. apply H0. auto.
destruct (eq_dec x y) as [E|E]; auto.
intros.
exists e; apply add_spec1; auto. apply E.eq_equiv; auto.
intros [H|(e',H)].
destruct E; auto.
exists e'; apply add_spec2; auto. intro; apply E.eq_equiv in H0; auto.
Qed.

Lemma add_neq_mapsto_iff : forall m x y e e',
 ~ E.eq x y -> (MapsTo y e' (add x e m)  <-> MapsTo y e' m).
Proof.
split; apply add_spec2; intro; apply E.eq_equiv in H0; auto.
Qed.
...
assert (~E.eq y x) by (intro; apply E.eq_equiv in H1; auto).
apply add_spec2 with (A:=elt) (m:=m) (v:=e')(v':=e) in H0; auto.
apply add_spec2; auto; intro; apply E.eq_equiv in H1. contradiction.
Qed.
...
Proof.
split; apply elements_spec1.
Qed.

Lemma elements_in_iff : forall m x,
 In x m <-> exists e, InA (@eq_key_elt _) (x,e) (elements m).
Proof.
unfold In; split; intros (e,H); exists e; apply elements_spec1; auto. 
Qed.

(* TODO: requires map, mapi *)
(*
...
apply mem_spec in H. apply empty_in_iff in H. 
contradiction.
Qed.
...
unfold eqb.
rewrite <- findA_NoDupA; intuition; try apply elements_spec2w; eauto.
Qed.
...
(* TODO: This feels hackish, is there a better way of doing things? *)

Lemma E_eq_refl: forall x : key, E.eq x x. Proof. apply E.eq_equiv. Qed.
Lemma E_eq_sym: forall x y : key, E.eq x y -> E.eq y x. Proof. apply E.eq_equiv. Qed.
Lemma E_eq_trans: forall x y z : key, E.eq x y -> E.eq y z -> E.eq x z.
...
(* TODO: this was "Arguments Equal {elt} m m'." but that made Coq complain -- is this fine?*)

Arguments Equal {A} m m'.

Add Parametric Relation (A : Type) : (t A) Equal
 reflexivity proved by (@Equal_refl A)
 symmetry proved by (@Equal_sym A)
 transitivity proved by (@Equal_trans A)
 as EqualSetoid.
...
Add Parametric Morphism A : (@MapsTo A)
...
Add Parametric Morphism elt : (@is_empty elt)
 with signature Equal ==> Logic.eq as is_empty_m.
Proof.
intros m m' Hm.
...
Add Parametric Morphism elt : (@mem elt)
 with signature E.eq ==> Equal ==> Logic.eq as mem_m.
Proof.
...
Add Parametric Morphism elt : (@find elt)
 with signature E.eq ==> Equal ==> Logic.eq as find_m.
Proof.
...
Add Parametric Morphism elt : (@add elt)
 with signature E.eq ==> Logic.eq ==> Equal ==> Equal as add_m.
Proof.
...
(* TODO ??? *)
(*
Module WFacts (M:WOps) := WFacts_fun M.E M.
Module Facts := WFacts.
*)
...
Module WProperties_fun (E:DecidableType)(M: WMapsOn E).
 ...
 Section Elt.
  ...
  Instance eqk_equiv : Equivalence eqk. Admitted.
  (* Proof. unfold eq_key; split; eauto. Qed. *)
 
  Instance eqke_equiv : Equivalence eqke. Admitted.
  (*
  Proof.
...
*)
  ...
  assert (forall a, ~ List.In a (elements m)).
   ...
   split; auto; split; auto. simpl; apply E.eq_equiv.
  ...
  Proof.
  rewrite <-elements_Empty; apply empty_spec.
  Qed.
  ...
  apply Hstep' with (of_list l); auto.
   rewrite InA_cons; left; red; auto. simpl; split; auto; apply E.eq_equiv.
  ...
  apply Rstep'; auto.
  destruct a; simpl; rewrite InA_cons; left; red; auto; simpl; split; auto; apply E.eq_equiv.
  Qed.
  ...
  Hypothesis Comp : Proper (E.eq==>Logic.eq==>eqA==>eqA) f.

  Lemma fold_init :
   forall m i i', eqA i i' -> eqA (fold f m i) (fold f m i').
  Proof.
  intros. apply fold_rel with (R:=eqA); auto.
  intros. apply Comp; auto. apply E.eq_equiv.
  Qed.
  ...
  apply Comp; auto; apply E.eq_equiv.
  apply Tra; auto.
  contradict Hnotin; rewrite <- Hnotin; exists e0; auto.
  Qed.

  Hint Resolve NoDupA_eqk_eqke NoDupA_rev elements_spec2w : map.
  ...
  *)
  ...
  Proof.
  intros; rewrite cardinal_spec, fold_spec.
  ...
  Proof.
  intros.
  rewrite cardinal_spec, elements_Empty.
  ...
  apply fold_Equal with (eqA:=Logic.eq); compute; auto.
  Qed.
  ...
  constructor; red; auto. simpl; split; auto; apply E.eq_equiv.
  Qed.
  ...
  Hypothesis Hf : Proper (E.eq==>Logic.eq==>Logic.eq) f.
  ...
  rewrite Hadd, add_mapsto_iff; auto.  left; split; auto; apply E.eq_equiv; auto.
  Qed.
  ...
  rewrite Hadd, add_mapsto_iff; auto. left; split; auto; apply E.eq_equiv.
  (* f k e = false *)
  ...
  assert (f k' e' = f k e). apply Hf; auto; apply E.eq_equiv; auto. congruence.
  exists (k',e'); auto.
  Qed.
  ...
  assert (H : MapsTo x e m').
   change (Equal m' (add x e m)) in Hadd; rewrite Hadd.
   apply add_spec1; auto; apply E.eq_equiv.
  ...
  apply MapsTo_spec with k'; auto; apply E.eq_equiv; auto.
  ...
  Lemma Partition_fold :
   ...
   Proper (E.eq==>Logic.eq==>eqA==>eqA) f ->
  ...
  apply Comp; auto using IH. apply E.eq_equiv.
  ...
  Proof.
  intros m m1 m2 Hm f.
  assert (Hf : Proper (E.eq==>Logic.eq==>Logic.eq) f).
  ...
  rewrite filter_iff.
  intuition.
  apply mem_spec in H; rewrite <- H1 in H; symmetry in H;
  apply no_fixpoint_negb in H; auto.
  apply not_mem_in_iff in H1; unfold negb; rewrite H1; reflexivity.
 ...
 Add Parametric Morphism elt : (@cardinal elt)
   with signature Equal ==> Logic.eq as cardinal_m.
