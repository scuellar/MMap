...
Module Type MSetRemoveMin (Import M:MMapInterface.S).

 Parameter remove_min : forall {A : Type}, t A -> option (key * A * t A).
 ...
 Axiom remove_min_spec1 : remove_min m = Some (k,v,m') ->
   min_elt m = Some (k, v) /\ Equal (remove k m) m'.
 Axiom remove_min_spec2 : remove_min m = None -> Empty m.
 End Spec.
...
Module Ops (X:Orders.OrderedType) <: MMapInterface.Ops X.
...
Include MMapGenTree.Ops X Color.

(* TODO: is it should be here or in the MMapGenTree file? *)
...
Definition singleton (k: key) (v: A) : tree A := Bk Leaf k v Leaf.

(** ** Changing root color *)

Definition makeBlack t : tree A :=
 match t with
 | Leaf => Leaf
 | Node _ a k v b => Bk a k v b
 end.

Definition makeRed t : tree A :=
 match t with
 | Leaf => Leaf
 | Node _ a k v b => Rd a k v b
 end.
...
Definition lbal l k v r : tree A :=
 match l with
 | Rd (Rd a k1 v1 b) k2 v2 c => Rd (Bk a k1 v1 b) k2 v2 (Bk c k v r)
 | Rd a k1 v1 (Rd b k2 v2 c) => Rd (Bk a k1 v1 b) k2 v2 (Bk c k v r)
 | _ => Bk l k v r
 end.

Definition rbal l k v r : tree A :=
 match r with
 | Rd (Rd b k1 v1 c) k2 v2 d => Rd (Bk l k v b) k1 v1 (Bk c k2 v2 d)
 | Rd b k1 v1 (Rd c k2 v2 d) => Rd (Bk l k v b) k1 v1 (Bk c k2 v2 d)
 | _ => Bk l k v r
 end.
...
Definition lbalS l k v r : tree A :=
 match l with
 | Rd a k1 v1 b => Rd (Bk a k1 v1 b) k v r
 | _ =>
   match r with
   | Bk a k2 v2 b => rbal' l k v (Rd a k2 v2 b)
   | Rd (Bk a k1 v1 b) k2 v2 c => 
        Rd (Bk l k v a) k1 v1 (rbal' b k2 v2 (makeRed c))
   | _ => Rd l k v r (* impossible *)
   end
 end.

Definition rbalS l k v r :=
 match r with
 | Rd b k1 v1 c => Rd l k v (Bk b k1 v1 c)
 | _ =>
   match l with
   | Bk a k1 v1 b => lbal (Rd a k1 v1 b) k v r
   | Rd a k1 v1 (Bk b k2 v2 c) => 
       Rd (lbal (makeRed a) k1 v1 b) k2 v2 (Bk c k v r)
   | _ => Rd l k v r (* impossible *)
   end
 end.
...
Fixpoint ins k v m :=
 match m with
 | Leaf => Rd Leaf k v Leaf
 | Node c l k' v' r =>
   match X.compare k k' with
   | Eq => Node c l k v r
   | Lt =>
     match c with
     | Red   => Rd (ins k v l) k' v' r
     | Black => lbal (ins k v l) k' v' r
     end
   | Gt =>
     match c with
     | Red   => Rd l k' v' (ins k v r)
     | Black => rbal l k' v' (ins k v r)
     end
   end
 end.

Definition add k v m := makeBlack (ins k v m).

(** ** Deletion *)

Fixpoint append (l:tree A) : tree A -> tree A :=
 match l with
 | Leaf => fun r => r
 | Node lc ll lk lv lr =>
   fix append_l (r:tree A) : tree A :=
   match r with
   | Leaf => l
   | Node rc rl rk rv rr =>
     match lc, rc with
     | Red, Red =>
       ...
       match lrl with
       | Rd lr' k v rl' => Rd (Rd ll lk lv lr') k v (Rd rl' rk rv rr)
       | _ => Rd ll lk lv (Rd lrl rk rv rr)
       end
     | Black, Black =>
       let lrl := append lr rl in
       match lrl with
       | Rd lr' k v rl' => Rd (Bk ll lk lv lr') k v (Bk rl' rk rv rr)
       | _ => lbalS ll lk lv (Bk lrl rk rv rr)
       end
     | Black, Red => Rd (append_l rl) rk rv rr
     | Red, Black => Rd ll lk lv (append lr r)
     end
   end
 end.

Fixpoint del k t : tree A :=
 match t with
 | Leaf => Leaf
 | Node _ a k' v' b =>
   match X.compare k k' with
   ...
   | Lt =>
     match a with
     | Bk _ _ _ _ => lbalS (del k a) k' v' b
     | _ => Rd (del k a) k' v' b
     end
   | Gt =>
     match b with
     | Bk _ _ _ _ => rbalS a k' v' (del k b)
     | _ => Rd a k' v' (del k b)
     end
   end
 end.

Definition remove k t : tree A := makeBlack (del k t).

(** ** Removing minimal element *)

Fixpoint delmin l k v r : (key * A * tree A) :=
 match l with
 | Leaf => (k,v,r)
 | Node lc ll lk lv lr =>
   let (min, l') := delmin ll lk lv lr in
   match lc with
   | Black => (min, lbalS l' k v r)
   | Red => (min, Rd l' k v r)
   end
 end.

Definition remove_min t : option (key * A * tree A) :=
 match t with
 | Leaf => None
 | Node _ l k v r =>
   let (min,t) := delmin l k v r in
   Some (min, makeBlack t)
 end.
...
Definition bogus : tree A * list (key * A) := (Leaf, nil).

Notation treeify_t := (list (key * A) -> tree A * list (key * A)).
...
Fixpoint plength_aux (l:list (key * A))(p:positive) := match l with
 | nil => p
...
Definition treeify (l:list (key * A)) :=
 fst (treeify_aux true (plength l) l).

(** ** Filtering *)

Fixpoint filter_aux (f: key -> A -> bool) m acc :=
 match m with
 | Leaf => acc
 | Node _ l k v r =>
...
Definition filter (f: key -> A -> bool) (m: t A) : t A :=
 treeify (filter_aux f m nil).

Fixpoint partition_aux (f: key -> A -> bool) m acc1 acc2 :=
 match m with
 | Leaf => (acc1,acc2)
 | Node _ sl k v sr =>
   let (acc1, acc2) := partition_aux f sr acc1 acc2 in
   if f k v then partition_aux f sl ((k, v)::acc1) acc2
   else partition_aux f sl acc1 ((k, v)::acc2)
 end.

Definition partition (f: key -> A -> bool) (m:t A) : t A * t A :=
  let (ok,ko) := partition_aux f m nil nil in
...
Fixpoint compare_height (s1x s1 s2 s2x: tree A) : comparison :=
 match skip_red s1x, skip_red s1, skip_red s2, skip_red s2x with
 | Node _ s1x' _ _ _, Node _ s1' _ _ _, Node _ s2' _ _ _, Node _ s2x' _ _ _ =>
   compare_height (skip_black s2x') s1' s2' (skip_black s2x')
 | _, Leaf, _, Node _ _ _ _ _ => Lt
 | Node _ _ _ _ _, _, Leaf, _ => Gt
 | Node _ s1x' _ _ _, Node _ s1' _ _ _, Node _ s2' _ _ _, Leaf =>
   compare_height (skip_black s1x') s1' s2' Leaf
 | Leaf, Node _ s1' _ _ _, Node _ s2' _ _ _, Node _ s2x' _ _ _ =>
...
Module Type MakeRaw (X:Orders.OrderedType) <: MMapInterface.RawMaps X.
Include Ops X.

(** Generic definition of binary-search-trees and proofs of
    specifications for generic functions such as mem or fold. *)

Include MMapGenTree.Props X Color.

Local Notation Rd := (Node Red).
Local Notation Bk := (Node Black).

(* TODO: Something is commented *)
